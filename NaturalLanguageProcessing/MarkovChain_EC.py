...
if __name__ == "__main__":
	args = sys.argv
	if len(args) < 6:
		print("Error: run command : MarkovChain_EC.py AUTH-DIR-1/ AUTH-DIR-2/ PROB-FILE-1 PROB-FILE-2 RESULT-FILE")
		sys.exit()

	auth_dir1 = str(args[1])
	auth_dir2 = str(args[2])
	prob_file1 = str(args[3])
	prob_file2 = str(args[4])
	res_file = open(str(args[5]),'a')
	...
	ngram_auth1 = ngram(stop_words)
	ngram_auth1.read_and_tokenize(auth_dir1)
	ngram_auth1.ngram_count()
	ngram_auth1.compute_prob(prob_file1)

	res_file.write("\n--------- Author 1 ----------")
	for i in range(10):
		seq, prob = ngram_auth1.generate_seq()
		res_file.write("\n\n"+str(i+1)+". "+seq+"\n")
		res_file.write("Probability : ")
		json.dump(prob, res_file)

	ngram_auth2 = ngram(stop_words)
	ngram_auth2.read_and_tokenize(auth_dir2)
	ngram_auth2.ngram_count()
	ngram_auth2.compute_prob(prob_file2)

	res_file.write("\n\n--------- Author 2 ----------")
	for i in range(10):
		seq, prob = ngram_auth2.generate_seq()
		res_file.write("\n\n"+str(i+1)+". "+seq+"\n")
		res_file.write("Probability : ")
		json.dump(prob, res_file)
